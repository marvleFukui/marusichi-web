...
- 左メニュー **「求人情報」→ 新規追加**。**タイトル**（一覧に表示）＋**求人カテゴリー**（新卒/中途/説明会/インターンシップ ※編集可）を選択。
- **求人情報テーブル（ACF・繰り返し）**：「見出し」＝項目名（例：募集職種／給与／勤務地）、「本文」＝内容。→ **詳細ページに表として表示**。
- 反映先：**TOP と RECRUIT ページの「RECRUIT INFO」** に最新3件が自動表示（カテゴリ・日付・タイトル→詳細へ）。「MORE」＝求人情報一覧 `/jobinfo/`。
- 詳細 URL＝`/jobinfo/<スラッグ>/`、一覧＝`/jobinfo/`。**日本語スラッグは英数字に手動修正**（施工実績と同じ注意点）。
- ※ **パーマリンクを再保存**（設定→パーマリンク）しないと `/jobinfo/` が404になります。
